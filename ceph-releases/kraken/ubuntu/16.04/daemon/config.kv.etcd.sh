...
function get_mon_config {
  ...
  if etcdctl $ETCDCTL_OPT ${KV_TLS} get ${CLUSTER_PATH}/monSetupComplete; then
    ...
    etcdctl $ETCDCTL_OPT ${KV_TLS} set ${CLUSTER_PATH}/bootstrapOsdKeyring < /var/lib/ceph/bootstrap-osd/${CLUSTER}.keyring
    etcdctl $ETCDCTL_OPT ${KV_TLS} set ${CLUSTER_PATH}/bootstrapMdsKeyring < /var/lib/ceph/bootstrap-mds/${CLUSTER}.keyring
    etcdctl $ETCDCTL_OPT ${KV_TLS} set ${CLUSTER_PATH}/bootstrapRgwKeyring < /var/lib/ceph/bootstrap-rgw/${CLUSTER}.keyring
    uuencode /etc/ceph/monmap-${CLUSTER} | etcdctl $ETCDCTL_OPT ${KV_TLS} set ${CLUSTER_PATH}/monmap
